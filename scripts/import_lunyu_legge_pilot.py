...
ALIGNMENTS_PATH = REPO_ROOT / "corpus" / "processed" / "alignments" / "lunyu__book-01-xueer__zhwikisource-20260529__legge-cc-v1-1893__alignments.jsonl"
...
def _load_sources(rights_ids: set[str]) -> list[dict[str, object]]:
    sources = load_json_compatible_yaml(METADATA_DIR / "sources.yml")
    for source in sources:
        if source["rights_status"] not in rights_ids:
            raise ValueError(f"Unknown rights status for source {source['source_id']}: {source['rights_status']}")
...
def import_pilot(db_path: Path | str = DEFAULT_DB_PATH) -> dict[str, object]:
    initialize_database(db_path)

    works = load_json_compatible_yaml(METADATA_DIR / "works.yml")
    sections = load_json_compatible_yaml(METADATA_DIR / "sections.yml")
    rights = load_json_compatible_yaml(METADATA_DIR / "rights.yml")
    rights_ids = {item["rights_id"] for item in rights}
    persons = _load_people()
    sources = _load_sources(rights_ids)
    segments = _load_segments(sources)
    alignments = read_jsonl(ALIGNMENTS_PATH)
    ...
    with connect_db(db_path) as connection:
        ...
        tracked_paths.append(ALIGNMENTS_PATH)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Import the Lunyu plus Legge public-domain pilot into SQLite.")
    parser.add_argument("--db", default=str(DEFAULT_DB_PATH), help="Path to the SQLite database file.")
    args = parser.parse_args()

    summary = import_pilot(args.db)
