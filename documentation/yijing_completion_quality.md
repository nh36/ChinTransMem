# Yijing completion quality

Generated from `logs/qc_reports/yijing__alignment_qc.json`.

## Summary

- Work state: active
- Total detected sections: 64
- Active sections: 64
- Exportable sections: 64
- English witness: ChineseNotes bilingual mirror of James Legge, Book of Changes (1882)
- Exact alignments: 450
- Automatic fine-grained alignments: 450
- Curated override sections: 0
- Remaining coarse fallbacks: 0
- Blocked sections: 0
- Hard failures: 0
- Corruption issues before repair: 0
- Corruption issues corrected: 0
- Corruption issues remaining: 0
- Alignment drift checks run: 64
- Drift issues before repair: 0
- Drift issues repaired: 0
- Drift issues remaining: 0
- Canonical line-order checks run: 64
- Line-order issues before repair: 1
- Line-order issues repaired: 1
- Line-order issues remaining: 0

## Alignment granularity

- block: 450

## Remaining fallbacks

- None.

## Curated override sections

- None.

## Blocked sections

- None.
